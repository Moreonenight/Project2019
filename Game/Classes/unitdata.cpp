...
bool unitdata::initial(string &datapath)
{
	...
	for (order = 0; order < NUM_OF_KINDS; order++) {
		...
		cocos2d::SpriteFrameCache::getInstance()->addSpriteFramesWithFile((const string)buf);
		buf.erase();
		for (curDir = 0; curDir < 4; curDir++) {
			for (index = 0; index < numOfFrames[order]; index++) {
					fgets((char*)buf.c_str(), 100, data);
					if (numOfFrames[order] == 1) continue;
					else (&animations[order][curDir])->addSpriteFrameWithFile(buf);
					buf.erase();
				}
	...
	maxHp = m2["maxHp"]; maxMana = m2["maxMana"]; damage = m2["damage"]; 
	ASPD = m2["ASPD"];/* epl = m2["epl"];*/ initLvl = m2["initLvl"]; initGold = m2["initGold"];
	attackRange = m2["attackRange"]; ammoSpeed = m2["ammoSpeed"];
	fclose(data);
